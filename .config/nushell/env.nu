...
use ~/dotfiles/nushell/completions/pnpm-compeletions.nu *
use ~/dotfiles/nushell/completions/uv-completions.nu *
use ~/dotfiles/nushell/completions/tailscale-completion.nu *

# 環境変数を.envファイルから読み込む関数
def load-env-file [path: string] {
  open $path
  | lines
  ...
  | each {|line|
      ...
      if (($parts | length) < 2) {
        ...
        {
          key: ($parts | get 0 | str trim)
          value: ($parts | get 1 | str trim | str trim --char '"' | str trim --char "'")
        }
      }
    }
  | compact
