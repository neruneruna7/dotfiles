...
-- キーバインド無効化トグルstart
local toggle_wezterm_keys_action

-- 状態表示の更新周期。単位はミリ秒。
config.status_update_interval = 1000


local function wezterm_keys_disabled(window)
    local overrides = window:get_config_overrides() or {}
    return overrides.disable_default_key_bindings == true
end

local function render_keybind_status(window)
    local disabled = wezterm_keys_disabled(window)

    if disabled then
        window:set_right_status(wezterm.format({
            { Background = { Color = NierColor.nier_beige_light } },
            { Foreground = { Color = NierColor.nier_beige_dark } },
            { Text = " WEZ:LOCKED " },
        }))
    else
        window:set_right_status(wezterm.format({
            { Background = { Color = NierColor.nier_beige_semi_dark } },
            { Foreground = { Color = NierColor.nier_beige_light } },
            { Text = " WEZ:ON " },
        }))
    end
end

local function toggle_wezterm_keys(window, pane)
    local overrides = window:get_config_overrides() or {}

    if overrides.disable_default_key_bindings == true then
        -- WezTerm キーバインドを復帰
        overrides.disable_default_key_bindings = nil
        overrides.keys = nil
    else
        -- WezTerm キーバインドを一時無効化
        -- 復帰用キーだけは残す
        ...
        overrides.keys = {
            {
                key = "F12",
                mods = "CTRL|SHIFT",
                action = toggle_wezterm_keys_action,
            },
            {
                key = "v",
                mods = "CTRL|SHIFT",
    ...
    -- 次の周期更新を待たず、即座に表示更新する
    render_keybind_status(window)
end



toggle_wezterm_keys_action = wezterm.action_callback(toggle_wezterm_keys)
